...
object DataDummy{
    ...
    fun generateDummyTV():List<TvEntitys>{
        ...
        val poster = arrayOf(
            ...
            "https://image.tmdb.org/t/p/w780/htJzeRcYI2ewMm4PTrg98UMXShe.jpg",
        )
    ...
    fun generateDetailMovie():List<MovieDetailEntity>{
        ...
        for (position in title.indices){
            val itemMovie = MovieDetailEntity(
                ...
                id = id[position],
                posterPath = posterMovie[position],
                original_title = title[position],
                name_genre = title[position]
            )
            movies.add(itemMovie)
        }
        return movies
    ...
    fun generateDetailDummyTV():List<TvDetailEntity>{
        val tvDetail = ArrayList<TvDetailEntity>()
    ...
    fun generateRemoteDetailMovie():List<MovieDetailResponse>{
        ...
        for (position in title.indices){
            val itemMovie = MovieDetailResponse(
                overview = overview[position],
                releaseDate = dateRelease[position],
                popularity = popularity[position],
                title = title[position],
    ...
    fun generateRemoteDetailDummyTV(): List<TvDetailResponse> {

        val tvDetail = ArrayList<TvDetailResponse>()
    ...
    fun generateDummyMovieWithDetail(movie:MovieDetailEntity) =
        MovieWithDetail(movie, generateDummyMovie(),null, generateUpcominglDummyMovie())

    fun generateDummyTvWithDetail(tv:TvDetailEntity) =
        TvWithDetail(tv, generateDummyTV(), null, generateUpcominglDummyTv())
}