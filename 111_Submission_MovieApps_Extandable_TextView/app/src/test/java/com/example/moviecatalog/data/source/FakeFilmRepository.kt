...
class FakeFilmRepository constructor(
    ...
    override fun getMovieSearch(query: String): LiveData<List<MovieSearchEntity>> {
        val searchMovie = MutableLiveData<List<MovieSearchEntity>>()

        remoteDataSource.getMovieSearch(query, object : RemoteDataSource.LoadMovieSearchCallback{
            override fun onAllLoadMovieSearch(movieSearch: List<ResultsItemSearchMovie>) {
                val search = ArrayList<MovieSearchEntity>()

                for (response in movieSearch){
                    val moviesSearch = MovieSearchEntity(
                        response.releaseDate,
                        response.popularity,
                        response.id,
                        response.title,
                    ...
                    search.add(moviesSearch)
                }
                searchMovie.postValue(search)
            }
        })
        return searchMovie
    }

    override fun getTvSearch(query: String): LiveData<List<TvSearchEntity>> {
        val searchTv = MutableLiveData<List<TvSearchEntity>>()

        remoteDataSource.getTvSearch(query, object : RemoteDataSource.LoadTvSearchCallback{
            override fun onAllLoadTvSearch(tvSearch: List<ResultsItemSearchTv>) {
                val search = ArrayList<TvSearchEntity>()

                for (response in tvSearch){
                    val tvSearchs = TvSearchEntity(
                        response.popularity,
                    ...
                    search.add(tvSearchs)
                }
                searchTv.postValue(search)
            }
        })
