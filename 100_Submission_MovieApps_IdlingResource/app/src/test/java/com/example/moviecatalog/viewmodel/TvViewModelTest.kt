...
class TvViewModelTest {
    ...
    @Mock
    private lateinit var observerTv:Observer<List<TvEntitys>>

    @Mock
    private lateinit var observerUpTv:Observer<List<UpcomingTvEntity>>
    ...
    fun getTv() {
        ...
        val tv = MutableLiveData<List<TvEntitys>>()
        tv.value = dummyTv

        `when`(filmRepository.getAllTv()).thenReturn(tv)
        val tvEntitys = viewModel.getTvPop().value
    ...
    @Test
    fun getUpcomingTv(){
        val dummyUpTv = DataDummy.generateUpcominglDummyTv()
        val upTv = MutableLiveData<List<UpcomingTvEntity>>()
        upTv.value = dummyUpTv

        `when`(filmRepository.getUpcomingTv()).thenReturn(upTv)
        val upTvEntity = viewModel.getUpcomingTv().value
