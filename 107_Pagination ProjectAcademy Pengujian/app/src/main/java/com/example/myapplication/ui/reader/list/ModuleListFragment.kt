...
import com.example.myapplication.ui.reader.CourseReaderCallback
...
class ModuleListFragment : Fragment(), MyAdapterClickListener {
    ...
    override fun onActivityCreated(savedInstanceState: Bundle?) {
        ...
        viewModel.modules.observe(this, Observer{ modulesEntity ->
            if(modulesEntity != null){
                when(modulesEntity.status){
                    Status.LOADING -> progress_bar.visibility = View.VISIBLE
                    Status.SUCCESS -> {
                        progress_bar.visibility = View.GONE
                        populateRecyclerView(modulesEntity.data as List<ModuleEntity>)
                    }
                    Status.ERROR -> {
                        progress_bar.visibility = View.GONE
